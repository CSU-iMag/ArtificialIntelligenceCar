        ...
        for (; i_out_x < dim_im_out_x - padding_x; i_out_x++)
        {
                 ...
                 i_ker_y++)
            {
				...
				if (i_ker_y < 0 || i_ker_y >= dim_im_in_y) {
					memset(pBuffer, 0, sizeof(q15_t)*ch_im_in * dim_kernel_x);
					pBuffer += ch_im_in * dim_kernel_x;
					continue;
				}
				i_ker_x = i_out_x * stride_x - padding_x;
				if (i_ker_x < 0 || i_ker_x + dim_kernel_x >= dim_im_in_x) 
				{
					for (; i_ker_x < i_out_x * stride_x - padding_x + dim_kernel_x; i_ker_x++)
					{
						if (i_ker_x < 0 || i_ker_x >= dim_im_in_x)
						{
					...
					arm_q7_to_q15_reordered_no_shift((q7_t *) Im_in
													 ...
													 (i_ker_y *
													  dim_im_in_x +
													  i_out_x *
													  stride_x - padding_x) * ch_im_in, pBuffer, ch_im_in * dim_kernel_x);
					pBuffer += ch_im_in * dim_kernel_x;
				}				
