...
//  @return     void
//  Sample usage:
//  如果需要使用周期中断、延时、计时等功能都需要对PIT进行初始化之后才能调用相应功能函数
//-------------------------------------------------------------------------------------------------------------------
void pit_init(void) {
...
//  @return     void
//  Sample usage:
//  本函数通常用户无需关心，如果需要使用周期中断功能，应该调用h文件的宏定义函数（带有时间单位）
//-------------------------------------------------------------------------------------------------------------------
void pit_interrupt(pit_chnl_t pit_ch, uint32 count) {
    PIT_SetTimerPeriod(PIT, pit_ch, count);
    PIT_EnableInterrupts(PIT, pit_ch,
                         kPIT_TimerInterruptEnable); //打开PIT通道0中断
    PIT_SetTimerChainMode(PIT, pit_ch, false);
...
//  @return     uint32      计数值
//  Sample usage:
//  本函数通常用户无需关心，如果需要获取计数值，应该调用h文件的宏定义函数（带有时间单位）
//-------------------------------------------------------------------------------------------------------------------
uint32 pit_get(pit_chnl_t pit_ch) {
    if (PIT_GetStatusFlags(PIT, pit_ch)) {
        PIT_ClearStatusFlags(PIT, pit_ch, kPIT_TimerFlag);
        return 0XFFFFFFFF;
        //计时溢出 返回最大值
    }
    return (0XFFFFFFFF - PIT_GetCurrentTimerCount(PIT, pit_ch));
}
